 ...
 * Lets the player activate inactive departments directly from Home.
 */
    ...
    <>
      ...
      <div className="hire-modal" role="dialog" aria-modal="true" aria-label="Hire an employee">
              ...
              {DEPARTMENTS_DATA.map((d) => {
                ...
                return (
                  <option key={d.id} value={d.id}>
                    {d.icon} {d.name}{total === 0 ? ' (inactive)' : ' (' + total + ' staff)'}
                  </option>
                )
              })}
            </select>
        ...
        <div className="hire-modal__actions">
          ...
          <button className="btn btn--primary btn--md" onClick={handleHire}>Hire {type === 'specialist' ? 'Specialist' : 'Consultant'}</button>
        </div>
      </div>
    </>
  )
