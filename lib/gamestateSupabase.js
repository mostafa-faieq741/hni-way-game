...
export async function allPlayerStats() {
  ...
  const { data, error } = await db.from('players')
    .select('player_id, display_name, role, game_state(cash, reputation, net_profit, total_revenue, game_status, qualified)')
    .eq('role', 'player')
  if (error) throw new Error(error.message)
  return (data || []).map((p) => {
    const g = Array.isArray(p.game_state) ? p.game_state[0] : p.game_state
    return {
